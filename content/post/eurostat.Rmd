...
The Eurostat package does not, currently, provide that functionality but a useful tip in the github issues from [Joona Lehtomäki](@jlehtoma) resolved this. The resolution obtained  (1:10million) is sufficient for this example wothout looking crude
...
country_pop <- pop %>% 
  filter(nchar(as.character(geo_code))==2)  %>% 
  rename(pop=values)
...
accidents_geo <- geo_join(res10,df15,  by_sp="NUTS_ID", by_df="geo_code", how = "inner") 
